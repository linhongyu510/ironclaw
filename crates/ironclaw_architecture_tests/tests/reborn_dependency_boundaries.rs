...
fn reborn_runner_llm_wiring_is_isolated() {
    ...
    assert!(
        !root
            .join("crates/ironclaw_turn_runner/src/model_gateway.rs")
            .exists(),
...
fn reborn_internal_crate_keeps_directory_of_modules_lib_rs() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/ironclaw_turn_runner/src/lib.rs"))
    ...
    let composition_capability_host =
        root.join("crates/ironclaw_composition/src/runtime/capability_host.rs");
    assert!(
...
fn reborn_provider_catalog_is_owned_by_its_crate() {
        ...
        while let Some(current) = stack.pop() {
            ...
            for entry in entries.flatten() {
                ...
                if !relative.starts_with("crates/ironclaw_llm/")
                    && !relative.starts_with("crates/ironclaw_architecture_tests/")
                {
    ...
    let init_rs =
        std::fs::read_to_string(root.join("crates/ironclaw_cli/src/commands/config/init.rs"))
...
fn hosted_mcp_discovery_is_never_driven_by_ambient_startup_composition() {
    let root = workspace_root();
    let factory = std::fs::read_to_string(root.join("crates/ironclaw_composition/src/factory.rs"))
