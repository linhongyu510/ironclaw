...
pub(crate) async fn build_filesystem(
    ...
    sandbox_workspace_users_root: Option<&Path>,
    ...
    if let Some(users_root) = sandbox_workspace_users_root {
...
/// Mount the shared parent of sandbox user workspaces once. Per-invocation
/// mount views narrow access to one digest leaf, while `mount_local_per_leaf`
/// keeps symlink containment pinned to that leaf rather than the shared root.
pub(crate) fn mount_sandbox_user_workspace_root(
