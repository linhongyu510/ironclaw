...
mod tests {
    ...
    async fn hosted_single_tenant_volume_sandboxed_workspace_resolves_per_invocation_scope() {
        ...
        let local_runtime = services
            .local_runtime_for_test()
            .expect("local runtime substrate"); // safety: test-only assertion in #[cfg(test)] module.
        let sandbox_workspaces_root = local_runtime
            .sandbox_workspaces_root
            .clone()
            .expect("sandboxed profile always sets a sandbox workspaces root"); // safety: test-only assertion in #[cfg(test)] module.
        let policy = Arc::new(
            crate::builtin_capability_policy::builtin_capability_policy().expect("policy parses"),
        );
        let capability_io = Arc::new(StagedCapabilityIo::default());
        ...
        let factory = RefreshingLoopCapabilityPortFactory {
            ...
            sandbox_workspaces_root: Some(sandbox_workspaces_root.clone()),
        ...
        let workspace_path_a = key_a.workspace_path(&sandbox_workspaces_root);
        let workspace_path_b = key_b.workspace_path(&sandbox_workspaces_root);
        assert_ne!(
