...
fn with_clean_attested_env(body: impl FnOnce()) {
    ...
    unsafe {
        for key in ATTESTED_ENV_VARS {
            std::env::remove_var(key); // env-hermetic: serialized through ENV_LOCK in with_clean_attested_env
        }
    }
    ...
    unsafe {
        for (key, value) in saved {
            match value {
                Some(v) => std::env::set_var(key, v), // env-hermetic: serialized through ENV_LOCK in with_clean_attested_env
                None => std::env::remove_var(key), // env-hermetic: serialized through ENV_LOCK in with_clean_attested_env
            }
        }
    }
