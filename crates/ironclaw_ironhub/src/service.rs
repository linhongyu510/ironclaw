...
impl IronHubService {
    ...
    async fn update(
        ...
        let result = async {
            ...
            let (kind, provenance, artifact_digest) = classify_gate_and_digest(
                &manifest,
                name,
                options.kind,
                &install_options,
            ...
            let catalog_origin = private_origin
                ...
                .map(Ok)
                .unwrap_or_else(|| catalog_origin_for_url(&self.manifest_url))?;
            let target_provenance = registry_provenance(
                &manifest,
                kind,
                name,
                &artifact_digest,
            ...
            let (lifecycle, active) = match kind {
                ...
                IronHubEntryKind::Skill => {
                    ...
                    let metadata = self
                        ...
                        .ok_or_else(|| {
                            invalid(format!(
                                "skill '{name}' has no durable install metadata"
                            ))
                        })?;
...
fn ironhub_command_capability_id(
    ...
    let value = match command {
        ...
        IronHubCommand::Status { .. } => crate::IRONHUB_STATUS_CAPABILITY_ID,
        IronHubCommand::Install { .. } => crate::IRONHUB_INSTALL_CAPABILITY_ID,
        IronHubCommand::Update { .. } => crate::IRONHUB_UPDATE_CAPABILITY_ID,
    };
