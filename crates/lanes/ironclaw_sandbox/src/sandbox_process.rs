...
// Retain 96 bits of the released SHA-256 workspace identity in provider and
// Docker names: enough collision resistance while staying within name limits.
const SANDBOX_USER_NAME_DIGEST_PREFIX_LEN: usize = 24;

pub(super) fn sandbox_user_container_name(key: &TenantUserWorkspaceKey) -> String {
    let digest = key.digest_segment();
    let prefix = digest
        .get(..SANDBOX_USER_NAME_DIGEST_PREFIX_LEN)
        .unwrap_or(digest);
