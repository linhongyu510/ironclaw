...
fn manifests() -> Result<Vec<CapabilityManifest>, ExtensionError> {
    let manifests = vec![
        ...
        lifecycle_manifest(
            EXTENSION_REGISTER_HOSTED_MCP_CAPABILITY_ID,
            "Register a custom hosted MCP endpoint as a private inactive extension installation for the calling user. First call builtin.extension_search; use this only when the requested MCP server is not already registered for that user. Choose auth_type from provider documentation or explicit user context: no_auth only for a documented public endpoint, bearer for a static API token or PAT sent as a Bearer credential, and oauth for a browser authorization-code flow. If the auth type is unclear, ask the user instead of guessing. On success, pass the returned package_ref.id to builtin.extension_install to attempt activation. Registration never activates the extension or grants membership to another user.",
            vec![
