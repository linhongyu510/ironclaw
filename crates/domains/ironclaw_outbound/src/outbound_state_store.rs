...
//! - `/outbound/notification-inbox/state.json` — tenant/user-scoped,
//!   metadata-only notification records with read, resolved, and archived
//!   lifecycle timestamps. Raw message or model content is never stored.
...
#[async_trait]
impl<F> NotificationInboxStorePort for OutboundStateStore<F>
where
    F: RootFilesystem,
{
    async fn publish(
        ...
        let resource_scope = notification_resource_scope(&request.recipient);
        ...
        cas_update(
            ...
            &resource_scope,
            &path,
            decode_notification_inbox_snapshot,
            encode_notification_inbox_snapshot,
            move |current: Option<NotificationInboxSnapshot>| {
                ...
                async move {
                    ...
                    validate_notification_inbox_snapshot(&snapshot, &recipient)?;
        ...
        .await
        .map_err(map_notification_cas_error)
    }
    ...
    async fn list(
        ...
        if request.limit == 0 || request.limit > crate::NOTIFICATION_PAGE_LIMIT_MAX {
            return Err(OutboundError::InvalidRequest {
        ...
        let Some(snapshot) = self
            .get_json::<NotificationInboxSnapshot>(&resource_scope, &path)
            .await?
        else {
        ...
        validate_notification_inbox_snapshot(&snapshot, &request.recipient)?;
    ...
    async fn mark_read(&self, request: NotificationMutationRequest) -> Result<(), OutboundError> {
        mutate_notification(self, request, |record, occurred_at| {
            record
                .read_at
                .get_or_insert(occurred_at.max(record.created_at));
        })
        .await
    }
    ...
    async fn resolve(&self, request: NotificationMutationRequest) -> Result<(), OutboundError> {
        mutate_notification(self, request, |record, occurred_at| {
            ...
            record.resolved_at.get_or_insert(occurred_at);
            record.read_at.get_or_insert(occurred_at);
        })
        .await
    }
    ...
    async fn archive(&self, request: NotificationMutationRequest) -> Result<(), OutboundError> {
        mutate_notification(self, request, |record, occurred_at| {
            let occurred_at = occurred_at.max(record.created_at);
            record.archived_at.get_or_insert(occurred_at);
            record.read_at.get_or_insert(occurred_at);
        })
        .await
    }
...
async fn mutate_notification<F, M>(
    store: &OutboundStateStore<F>,
    request: NotificationMutationRequest,
    mutation: M,
) -> Result<(), OutboundError>
where
    F: RootFilesystem,
    M: Fn(&mut NotificationRecord, DateTime<Utc>) + Send + Sync + Clone + 'static,
{
    let resource_scope = notification_resource_scope(&request.recipient);
    let path = notification_inbox_path()?;
    let recipient = request.recipient.clone();
...
fn notification_inbox_path() -> Result<ScopedPath, OutboundError> {
    ScopedPath::new(NOTIFICATION_INBOX_PATH).map_err(|_| OutboundError::Backend)
}
...
fn validate_notification_source(source: &crate::NotificationSource) -> Result<(), OutboundError> {
...
fn validate_notification_action(
    ...
    action: &crate::NotificationAction,
) -> Result<(), OutboundError> {
    match action {
        crate::NotificationAction::OpenThread { thread_id } if thread_id != &source.thread_id => {
            Err(OutboundError::InvalidRequest {
                reason: "notification action does not match its source thread",
            })
        }
        crate::NotificationAction::OpenThread { .. } => Ok(()),
    }
}

fn validate_notification_inbox_snapshot(
    ...
    for record in &snapshot.notifications {
        let lifecycle_timestamp_is_invalid =
            [record.read_at, record.resolved_at, record.archived_at]
                .into_iter()
                .flatten()
        ...
        if &record.recipient != recipient
            ...
            || lifecycle_timestamp_is_invalid
...
fn decode_notification_inbox_snapshot(
    bytes: &[u8],
) -> Result<NotificationInboxSnapshot, OutboundError> {
    serde_json::from_slice(bytes).map_err(|_| OutboundError::Serialization)
}

fn encode_notification_inbox_snapshot(
    snapshot: &NotificationInboxSnapshot,
) -> Result<Entry, OutboundError> {
    validate_notification_inbox_snapshot(snapshot, &snapshot.recipient)?;
    let body = serde_json::to_vec(snapshot).map_err(|_| OutboundError::Serialization)?;
    Ok(Entry::bytes(body)
        .with_content_type(ContentType::json())
        .with_indexed(
            tenant_id_index_key(),
            tenant_id_index_value(&snapshot.recipient.tenant_id),
        ))
}
