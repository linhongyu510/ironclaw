...
| `suggestions` | Suggestion-card read and generate routes (#7038): thin `ProductSurface` pass-throughs, generation state derived server-side | Generation logic itself — that is `ironclaw_assistant`'s `SuggestionsProductService` | `get_suggestions`, `generate_suggestions` |
