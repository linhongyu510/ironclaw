//! Deployment-wide filesystem persistence for skill-learning settings.
...
use ironclaw_product_contracts::operator_llm::{
    SkillLearningSettings, SkillLearningSettingsStore, SkillLearningSettingsStoreError,
};

const SETTINGS_PATH: &str = "/tenant-shared/llm-skill-learning.json";
...
pub struct FilesystemSkillLearningSettingsStore<F: RootFilesystem + ?Sized> {
    filesystem: Arc<ScopedFilesystem<F>>,
    scope: ResourceScope,
}

impl<F: RootFilesystem + ?Sized> FilesystemSkillLearningSettingsStore<F> {
    pub fn new(filesystem: Arc<ScopedFilesystem<F>>, scope: ResourceScope) -> Self {
        Self { filesystem, scope }
    }

    fn path() -> Result<ScopedPath, SkillLearningSettingsStoreError> {
        ScopedPath::new(SETTINGS_PATH).map_err(|_| SkillLearningSettingsStoreError::InvalidData)
    }
}

#[async_trait]
impl<F: RootFilesystem + ?Sized> SkillLearningSettingsStore
    for FilesystemSkillLearningSettingsStore<F>
{
    async fn read(&self) -> Result<Option<SkillLearningSettings>, SkillLearningSettingsStoreError> {
        ...
        {
            Ok(Some(bytes)) => bytes,
            Ok(None) => return Err(SkillLearningSettingsStoreError::InvalidData),
            Err(FilesystemError::NotFound { .. }) => return Ok(None),
            Err(error) => {
                tracing::error!(error = %error, "skill-learning settings read failed");
                return Err(SkillLearningSettingsStoreError::Unavailable);
            }
        };
        serde_json::from_slice(&bytes).map(Some).map_err(|error| {
            tracing::error!(error = %error, "skill-learning settings record is invalid");
            SkillLearningSettingsStoreError::InvalidData
        })
    }
    ...
    async fn write(
        ...
        let bytes = serde_json::to_vec(settings)
            .map_err(|_| SkillLearningSettingsStoreError::InvalidData)?;
        if bytes.len() > SETTINGS_MAX_BYTES {
            return Err(SkillLearningSettingsStoreError::InvalidData);
        }
        let path = Self::path()?;
        self.filesystem
            ...
            .await
            .map_err(|error| {
                tracing::error!(error = %error, "skill-learning settings write failed");
                SkillLearningSettingsStoreError::Unavailable
            })
    }
}
...
mod tests {
    ...
    fn store(
    ...
    ) -> FilesystemSkillLearningSettingsStore<InMemoryBackend> {
        ...
        FilesystemSkillLearningSettingsStore::new(Arc::new(filesystem), scope())
    }
    ...
    async fn old_record_shape_uses_defaults() {
        ...
        let path =
            FilesystemSkillLearningSettingsStore::<InMemoryBackend>::path().expect("settings path");
        store
            .filesystem
