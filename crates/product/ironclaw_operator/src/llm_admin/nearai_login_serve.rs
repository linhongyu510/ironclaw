...
pub fn nearai_callback_descriptor() -> Result<IngressRouteDescriptor, HostApiError> {
    ...
    // OAuthCallback + OAuthState + HostResolved is the host callback shape;
    // handler validation consumes one-time login state before credential mutation.
}
...
mod tests {
    ...
    fn nearai_callback_descriptor_records_state_guarded_effectful_workflow() {
        let descriptor = nearai_callback_descriptor().expect("valid descriptor");
