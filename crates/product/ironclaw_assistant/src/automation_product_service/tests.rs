...
fn deterministic_assessment_never_treats_missing_required_action_as_success() {
    ...
    let assessment = super::assess_run(&run, &required, Some(&[]));
...
#[tokio::test]
async fn automation_list_projects_required_capability_success_from_runtime_evidence() {
    ...
    let service = service_over(repo).with_run_evidence(Arc::new(
        super::ProjectedTriggerRunEvidenceSource::new(Arc::new(ReplayEventProjectionService::new(
            log,
        ))),
    ));
