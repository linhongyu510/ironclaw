...
| `notifications` | Authenticated durable-inbox listing and lifecycle mutations, including product-contract projection and error shaping | External-channel preferences or delivery — those are `outbound`; notification production belongs to the originating workflow | `notification_recipient`, `notification_mutation_request`, `map_notification_inbox_error`, `product_notification_kind`, `product_notification_severity` |
