...
`src/reborn_services.rs` is **7,354 lines** — the largest file in this crate —
...
| `notification-setup` | The generic per-channel notification-setup surface (§7b): dispatching the contracts-declared status view and enable/disable commands (`ironclaw_product_contracts::notification_setup` owns the descriptors) to the channel's adapter through `ChannelDeliveryResolver`, with fail-closed unknown-channel and requires-setup handling | Enrollment *behavior* and storage — the channel's own adapter owns those; push delivery stays with the delivery coordinator | `mod notification_setup`, `reborn_services/notification_setup.rs::ChannelNotificationSetupService`, `reborn_services/notification_setup.rs::UnsupportedChannelNotificationSetupService`, `reborn_services/notification_setup.rs::AdapterChannelNotificationSetupService`, `reborn_services/notification_setup.rs::SetupOperation`, `reborn_services/notification_setup.rs::project_status`, `reborn_services/notification_setup.rs::notification_setup_unavailable`, `reborn_services/notification_setup.rs::map_setup_channel_error` |
