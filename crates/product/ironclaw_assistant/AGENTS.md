...
| `ironhub` | The IronHub install-link handshake, and the link surface an operator reads and pastes the hub-minted shared key into | Extension installation itself — that is `extensions`; minting the key, which is IronHub's | `mod ironhub_link`, `reborn_services/ironhub_link.rs::ironhub_link_unavailable`, `reborn_services/ironhub_link.rs::map_ironhub_link_error`, `reborn_services/ironhub_link.rs::UnsupportedIronhubLinkAdminService` |
