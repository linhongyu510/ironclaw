...
| `suggestions` | Suggestion-card get/generate (#7038): the view/command descriptors and the `SuggestionsProductService` port whose real implementation (CAS claim / hidden-thread / synthetic-submit) lives in `suggestions_product_service.rs` | The derived-doc storage or generation logic itself — that is `ironclaw_suggestions` / `suggestions_product_service.rs` | `SUGGESTIONS_VIEW_ID`, `SUGGESTIONS_VIEW`, `SUGGESTIONS_GENERATE_COMMAND_ID`, `SUGGESTIONS_GENERATE_COMMAND`, `SuggestionsProductService`, `UnsupportedSuggestionsProductService` |
