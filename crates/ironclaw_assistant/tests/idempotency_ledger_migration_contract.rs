...
fn fingerprint(suffix: usize) -> ActionFingerprintKey {
    ActionFingerprintKey::new(
        ironclaw_assistant::ProductAdapterId::new("telegram").expect("adapter"),
        ironclaw_assistant::AdapterInstallationId::new("default-installation")
            .expect("installation"),
        ironclaw_assistant::ExternalActorRef::new("user", "telegram-user", None::<String>)
            .expect("actor"),
        SourceBindingKey::new("space:0:;conversation:6:chat-a;topic:0:;").expect("binding"),
        ironclaw_assistant::ExternalEventId::new(format!("event-{suffix:04}")).expect("event"),
    )
}
