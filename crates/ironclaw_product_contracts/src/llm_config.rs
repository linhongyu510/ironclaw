...
//! §6.1.3, §6.9.2).
//!
//! This is the product-facing contract the WebChat v2 Inference tab consumes
//! to list providers, add/edit/remove custom providers (including an API key),
//! pick the active provider+model, probe a provider (test connection / list
...
//!
//! Declaring it here rather than in `ironclaw_product` is what un-inverts the
//! ownership: `ironclaw_operator` is a *sibling* of product, not a consumer of
//! it, so the port it satisfies belongs at the boundary. Product keeps the
//! frozen `llm_config` view descriptor, the "no service wired" fail-closed
//! error, and the `RebornServices` wiring that calls through this port.
//!
