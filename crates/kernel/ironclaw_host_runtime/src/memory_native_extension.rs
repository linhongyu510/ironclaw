...
/// is actually constructible. `memory_assets` is passed in by composition for
/// the same reason as the mem0 bundle; `schema_assets` carries the provider's
/// own input schemas for refs the host does not compile in.
