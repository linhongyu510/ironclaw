...
- Add a first-party tool file per capability, except for the OMP coding family
  (read/write/edit/glob/grep), which shares the host-half file
  `first_party_tools/omp.rs` and is implemented per-engine in
  `ironclaw_extension_support::coding::omp`.
