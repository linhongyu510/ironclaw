...
Under `crates/extensions/packages/`, 17 package directories: 6 are workspace
crates (`slack`, `telegram`, `web-app`, `memory-native`, `mem0`, `mnesis`) and
11 are data-only (manifest + prompts/schemas, some with prebuilt WASM): github,
gmail, the five google-*, mnesis-rar, nearai-mcp, notion-mcp, web-access. Every package directory —
