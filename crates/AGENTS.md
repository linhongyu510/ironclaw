...
| `ironclaw_loop_host` | `ironclaw_loop_host/AGENTS.md`, `ironclaw_loop_host/CLAUDE.md` | Loop host support services: capability/input ports, allow sets, input queue, identity/skill context, cancellation; **system-prompt content assets** (`prompts/*.md` via `system_prompt_assets.rs`). | Owning core loop strategy or runtime lane execution; filesystem seeding of the on-disk `SYSTEM.md` (that stays in the composition root). |
