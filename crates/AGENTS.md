...
- Product adapter flow: `ironclaw_assistant` contracts and `adapter_registry` manifest projection -> `ironclaw_assistant` orchestration -> concrete adapter crate.
- Reborn binary/composition: `ironclaw_config` for boot config; `ironclaw_composition` for production wiring; the `ironclaw_reborn_cli/` directory (package `ironclaw`) for commands; `ironclaw_turn_runner` for standalone adapters/driver registry; `ironclaw_webui` for host-owned WebChat v2 listener lifecycle.
