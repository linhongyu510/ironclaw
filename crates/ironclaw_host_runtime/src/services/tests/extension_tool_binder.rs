...
fn first_party_test_package(service: &str, capability_id: &str) -> ExtensionPackage {
    ExtensionPackage::from_manifest(
        ExtensionManifest {
            ...
            runtime: ironclaw_extension_registry::ExtensionRuntime::FirstParty {
...
async fn binder_invokes_a_discovered_mcp_tool_through_the_tool_adapter() {
    ...
    let discovered = ironclaw_extension_registry::package_with_discovered_hosted_mcp_tools(
        &base,
        &[ironclaw_extension_registry::HostedMcpDiscoveredTool {
            name: "search".to_string(),
            description: "Discovered search tool".to_string(),
            input_schema: serde_json::json!({"type": "object"}),
            annotations: ironclaw_extension_registry::HostedMcpDiscoveredToolAnnotations {
                read_only_hint: true,
                ..Default::default()
            },
        }],
