...
#[tokio::test]
async fn rejects_egress_sensitive_unsupported_inputs_before_provisioning() {
    let cli = Arc::new(FakeRailwayCli::new());
    let transport = RailwayPreviewSandboxTransport::with_cli(config(), cli.clone());
    ...
    let mut background = request("tenant", "user", "true");
    background.background = true;
    assert_rejected(&transport, background).await;
