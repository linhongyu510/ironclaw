...
use crate::{
    ...
    process_output::sanitize_command_output_bytes, sandbox_process::RebornSandboxUserKey,
};
...
impl SandboxCommandTransport for RailwayPreviewSandboxTransport {
    async fn run_command(
        ...
        reject_unsupported_request_inputs(&request)?;
        ...
        let output_limit = request_output_limit(request.output_limit_bytes);
        ...
        self.checkpoint(&key, &sandbox_id, deadline, output_limit)
            .await?;
        let raw_output = combine_output(stdout, output.stderr);
        let output =
            sanitize_command_output_bytes(raw_output.as_bytes(), raw_output.clone()).preview;
...
impl RailwayCli for SystemRailwayCli {
    async fn execute(
        ...
        let result = tokio::time::timeout(timeout, async {
            ...
            if !status.success() {
                return Err(RuntimeProcessError::ExecutionFailed(
                    railway_cli_failure_message(operation, status.code(), &stderr, &environment),
                ));
            }
...
fn railway_cli_failure_message(
    ...
    let detail = sanitize_command_output_bytes(detail.as_bytes(), detail.clone()).preview;
    let exit = exit_code
...
fn request_output_limit(requested: Option<u64>) -> usize {
    requested
...
fn reject_unsupported_request_inputs(
    request: &CommandExecutionRequest,
) -> Result<(), RuntimeProcessError> {
    ...
    if request.background {
        return Err(RuntimeProcessError::ExecutionFailed(
            "Railway preview sandbox does not support background commands".into(),
        ));
    }
    Ok(())
}
    ...
    I: IntoIterator<Item = (String, String)>,
{
    ...
    let auth = variables
        ...
        .map(|value| ("RAILWAY_TOKEN", value))
        .or_else(|| {
            variables
                .get("RAILWAY_API_TOKEN")
                .filter(|value| !value.is_empty())
                .map(|value| ("RAILWAY_API_TOKEN", value))
        })
        .ok_or_else(|| {
            RuntimeProcessError::ExecutionFailed(
