...
impl VerifiedOriginConnector {
    ...
    fn from_root_store(root_store: rustls::RootCertStore) -> Result<Self, TlsInterceptError> {
        ...
        let mut client_config = rustls::ClientConfig::builder_with_provider(ring_crypto_provider())
        ...
        // leg is pinned to http/1.1 too, so a production connector never
        // lets an origin answer h2 on a leg whose decrypted bytes get
...
pub(crate) fn build_server_config(
    ...
    let mut server_config = rustls::ServerConfig::builder_with_provider(ring_crypto_provider())
        .with_safe_default_protocol_versions()
        .map_err(|error| {
    ...
    // this seam is not equipped to parse. See the
    // `h2_only_client_fails_the_handshake_with_no_application_protocol` /
    // `http1_1_client_still_completes_the_handshake` tests for the observed
    // fail-closed/happy-path behavior.
