...
// arch-exempt: large_file, pre-existing oversized file touched only to thread the optional lifecycle observer through planned-runtime parts, plan #6310
...
use ironclaw_loop_host::{
    ...
    LifecycleTrajectoryObserver, LoopAttachmentReadPort, LoopCapabilityPortDecorator,
...
pub struct AfterTurnHookDeps {
    ...
    /// The deployment's default agent/project axes, as the conversation path
    /// resolves them. A hook-started run acts on these axes and on the tenant
    /// and user the dispatch context carries — never on axes of its own.
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    pub lifecycle_trajectory_observer: Option<Arc<dyn LifecycleTrajectoryObserver>>,
