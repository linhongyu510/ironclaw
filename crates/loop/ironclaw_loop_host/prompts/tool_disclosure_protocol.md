...
2. Load the schemas of every promising result in a **single response** — do not spend one model turn per tool. Prefer one bulk call, `tool_describe(names=["<tool-a>", "<tool-b>", "<tool-c>"])`, which returns each schema as its own entry; a name that is wrong or unavailable fails only its own entry, so the rest still come back. One call takes at most 8 names — for more candidates than that, emit several `tool_describe` calls in parallel in the same response rather than spreading them across turns. `tool_describe(name="<tool>")` remains valid for a single tool. Describing tools one per round-trip is the one thing to avoid.
