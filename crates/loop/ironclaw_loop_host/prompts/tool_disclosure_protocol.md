...
2. Load the schemas of every promising result in a **single response** — do not spend one model turn per tool. Prefer one bulk call, `tool_describe(names=["<tool-a>", "<tool-b>", "<tool-c>"])`, which returns each schema as its own entry; a name that is wrong or unavailable fails only its own entry, so the rest still come back. `tool_describe(name="<tool>")` remains valid for a single tool, and emitting several `tool_describe` calls in parallel in one response is equally acceptable. Describing tools one per round-trip is the one thing to avoid.
