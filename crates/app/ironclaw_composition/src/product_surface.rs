...
use ironclaw_product_contracts::operator_llm::{LlmConfigService, SkillLearningSettingsStore};
...
pub(crate) async fn compose_llm_config_service(
    ...
    skill_learning_scope: ResourceScope,
) -> Option<(
    Arc<ironclaw_operator::RebornLlmConfigService>,
    Arc<ironclaw_extension_host::skill_learning::SkillLearningRuntimeControllerImpl>,
)> {
    let boot = boot?;
    let skill_learning_store = Arc::new(
        ironclaw_operator::FilesystemSkillLearningSettingsStore::new(
            Arc::clone(&scoped_filesystem),
            skill_learning_scope,
        ),
    );
    let controller = Arc::new(
        ironclaw_extension_host::skill_learning::SkillLearningRuntimeControllerImpl::default(),
    );
    match skill_learning_store.read().await {
        Ok(Some(settings)) if settings.enabled => {
            ...
            if valid {
                ironclaw_product_contracts::operator_llm::SkillLearningRuntimeController::apply(
                    controller.as_ref(),
                    settings,
                );
            } else {
                tracing::warn!(
                    "persisted skill-learning model is not available from the active provider; \
        ...
        Err(error) => {
            tracing::warn!(
                ?error,
                "skill-learning settings unavailable; keeping learning disabled"
            );
        }
    }
    ...
    let controller_port: Arc<
        dyn ironclaw_product_contracts::operator_llm::SkillLearningRuntimeController,
    > = controller.clone();
    let mut llm_config = ironclaw_operator::RebornLlmConfigService::new(boot.clone(), keys.clone())
        ...
        .with_skill_learning_store(skill_learning_store)
        .with_skill_learning_controller(controller_port);
