...
fn reborn_contracts_crates_carry_a_checked_size_ceiling() {
    ...
    const SIZE_CEILINGS: &[(&str, usize)] = &[
        ...
        // 16_167 -> 16_231 (IronHub agent link, merged with main): the
        // `IronhubLinkAdminService` port with status/set/clear, the
        // `ironhub_link` view and the set-key and clear-key command ids, and two
        // wire DTOs whose `Debug` redacts the key. Declaration only: the
        // implementation is
        // `ironclaw_extension_manager::ironhub::RebornIronhubLinkAdminService`.
        // The port is separate from `IronhubLinkService` because the admin
        // surface must answer before a key exists, while the register gateway is
        // `None` until one does. Count read from this gate.
        ("ironclaw_product_contracts", 16_231),
