...
/// ✎ Re-seeded 42_791 -> 42_977 on 2026-08-25 for the Mnesis retrieval
/// bootstrap (`mnesis_rar_bootstrap.rs`): boot-time credential bind plus
/// install/activate for the `[mcp]` retrieval package.
/// ✎ Re-measured -> 43_361 on 2026-08-26 merging main (#7770 curation wiring,
/// #7833, the subagent background-mode slices) into the Mnesis branch. Same
/// pair hazard as 08-21: the two parents' ceilings cannot be summed or max'd —
/// measured on the merged tree with
/// `bash scripts/ci/check-composition-budget.sh --print`; the manifest's
/// `loc_ceiling`/`loc_observed` move to the same figure in this commit. No new
/// composition code in the merge itself.
const COMPOSITION_ABSOLUTE_SRC_LOC: usize = 43_361;
