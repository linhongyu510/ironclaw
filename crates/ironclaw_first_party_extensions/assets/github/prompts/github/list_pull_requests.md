...
Do not use this capability for requests about "my" pull requests because it has no author filter. First call `github.get_authenticated_user`, then call `github.search_issues_pull_requests` with the returned login in `author` plus `type: "pr"` and the requested repository/state filters.
