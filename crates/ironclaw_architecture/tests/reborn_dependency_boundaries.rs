...
fn reborn_host_runtime_services_do_not_expose_lower_substrate_handles() {
    ...
    let obligations = std::fs::read_to_string(crate_path(
        &root,
    ...
    let scripts = std::fs::read_to_string(crate_path(&root, "crates/ironclaw_scripts/src/lib.rs"))
        .expect("script runtime lib.rs must be readable");
    let scripts_manifest =
        std::fs::read_to_string(crate_path(&root, "crates/ironclaw_scripts/Cargo.toml"))
