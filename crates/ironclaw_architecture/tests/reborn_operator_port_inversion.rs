...
//!
//! `ironclaw_operator` is a **sibling** of `ironclaw_product`, not a consumer
...
//! `ironclaw_product` inverted the ownership — the implementor had to compile
...
//! `ironclaw_operator` and `ironclaw_product` both declare `layer = "products"`
...
const COMPOSITION: &str = "ironclaw_reborn_composition";
...
/// `ironclaw_reborn_composition` for readiness status, which is the only layer
...
/// re-declared in `ironclaw_product`, which is how a "temporary compatibility
...
/// The operator crate's dependency on `ironclaw_product` is **gone, not
...
fn impl_scanner_reads_the_trait_out_of_real_impl_shapes() {
    let source = r#"
        ...
        impl ironclaw_product::LlmConfigService for RebornLlmConfigService {}
