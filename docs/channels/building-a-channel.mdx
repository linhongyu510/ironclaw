...
At runtime every package is discovered at the virtual package root `/system/extensions/<extension-id>/manifest.toml`. Users install discovered packages by ID: `ironclaw extension search` lists them, and `ironclaw extension install <extension-id>` installs one — it takes an extension ID from search results, not a filesystem path. The WebUI Extensions area exposes the same list/install/import/remove lifecycle. For local development, place the extension directory under `<reborn-home>/system/extensions/<extension-id>/`; a Reborn profile never adds a storage subdirectory.
