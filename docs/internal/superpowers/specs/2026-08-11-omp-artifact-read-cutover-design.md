# Omp Artifact Read Cutover Design
...
outputs through the pinned omp `read` surface instead.
...
needed portion with an ordinary omp read selector, for example:
...
run, matching omp's shared parent/subagent artifact manager. Stored bytes are
...
1. **The omp `read` tool is the only model-visible result recovery tool.**
...
2. **New large outputs are durable artifacts, not virtual wrappers over the
   existing result-record limit.** This is the full omp behavior, not a
   surface-only rename.
...
`FIRST_PARTY_MAX_OUTPUT_BYTES` is 1 MiB. The current omp handler therefore
cannot produce the larger artifact before the generic result writer sees it.

### Pinned omp behavior

At pinned commit `08819b279cf02ae2545e69dad7111ab48d91d35e`, omp has no
`result_read` tool.
...
`crates/extensions/ironclaw_extension_support/src/coding/omp/read.rs` currently
...
- Make `artifact://<numeric-id>` a complete pinned-omp read target, including
...
`artifact://` reference when the inline preview is incomplete. The omp `read`
...
### Route `artifact://` through omp read

The omp read flow becomes:
...
replay encounters old result_ref without artifact metadata
  ...
  -> omp read uses the artifact port
```
...
3. omp `read` resolves artifact URIs;
...
- Invalid artifact URI or selector: pinned omp model-correctable input error.
...
Extend `tests/fixtures/omp_coding_contract/` with pinned artifact URI cases:
...
Differential tests compare IronClaw output with pinned omp at commit
...
- The provider tool surface contains omp `read` and no `result_read` tool.
- Every incomplete inline result includes a readable `artifact://<numeric-id>`.
- Omp `read` matches pinned artifact selector, output, and error fixtures except
...
schemes), the remaining omp tools, or a WebUI artifact browser. Those remain
...
remaining omp surface are implemented and verified.
