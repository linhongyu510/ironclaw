...
├── ironclaw_telemetry        tenant BI telemetry persistence boundary (ADR 0005)
...
- **Persistence idiom:** `ScopedFilesystem` is the floor. Every domains crate is backend-neutral by construction, depending only on the filesystem substrate's virtual-path, mount, and compare-and-swap authority — never a database driver directly. A crate that instead needs a hand-written SQL backend is a deliberate, narrow design choice that must be justified by an ADR; `ironclaw_triggers` and `ironclaw_telemetry` are the domain-family exceptions, alongside `ironclaw_hooks` in the loop family. **The trigger and hooks ADRs are written and decided KEEP, and telemetry is now admitted by [`docs/internal/adr/0005-telemetry-keeps-dedicated-sql-tables.md`](../../../adr/0005-telemetry-keeps-dedicated-sql-tables.md).** They are exceptions for different reasons and should not be cited as one precedent: triggers' claim/lease semantics are not expressible on the fabric, hooks' backends are complete but unwired, and telemetry needs grouped upserts plus tenant/time indexes. Every exception still takes existing connection admission only: it owns its SQL and transactions but never creates a second pool or connection plane.
...
Inside the family, dependency edges are shallow and few: `ironclaw_conversations` depends on `ironclaw_triggers` for trusted-submission binding vocabulary; `ironclaw_attachments` depends on `ironclaw_extractors` for pure bytes-to-text transformation; `ironclaw_trace_commons` depends on `ironclaw_llm` to reuse its recording vocabulary. `ironclaw_telemetry` depends on the contracts-family telemetry membrane and the shared libSQL admission substrate, not on another domains crate. No other crate in the family depends on a sibling — the family's internal graph is a shallow forest, not a mesh.
...
- **Owns:**
  ...
  - ~~A filesystem-routed persistence path alongside~~ a dedicated SQL-backed persistence path, held under an ADR, for deployments that need it. ✎ **Corrected 2026-08-05 (guidance program, measured): there is no filesystem-routed path — this crate has no `ironclaw_filesystem` dependency and its `BoundaryRule` forbids one. ADR 0003's hand-written SQL over `ironclaw_libsql_runtime` is the persistence path, not one of two.**
  - An in-memory implementation for deterministic tests.
...
- **Depends on:** `ironclaw_host_api`. ✎ **Corrected 2026-08-05 (guidance program, measured): `ironclaw_prompt_envelope` is allowlisted for this crate but not depended on — the live normal set is exactly `{ironclaw_host_api}`, which is what keeps this crate a provider-neutral contract rather than a memory implementation.**
...
- **Purpose:** tenant BI telemetry persistence domain for model-safe observations, grouped hourly facts, and explicit export.
- **Owns:** the future telemetry record grammar, grouped SQL tables and tenant/time indexes, aggregation, and export adapters. This initial crate is an empty documented shell; behavior, migrations, and queues land separately.
...
- **Public surface:** future typed recorder and query contracts; no driver types escape private adapters.
- **Depends on:** `ironclaw_telemetry_contracts` and `ironclaw_libsql_runtime`; future PostgreSQL/libSQL drivers remain private and are admitted through existing composition wiring. It does not depend on filesystem, product, composition, or another domains crate.
...
- **Why a separate crate:** grouped upserts and indexed tenant/time range scans are not expressible by per-document CAS without rebuilding database semantics in the filesystem fabric. ADR 0005 records the narrow exception and its revisit conditions.
