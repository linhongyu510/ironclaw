...
Script and shell process execution deny ambient worker egress. Production user-sandbox profiles use a dedicated per-user `ironsh/iron-proxy` sidecar: the worker joins only an internal per-user Docker network configured with isolated gateway mode, so the Docker host has no bridge endpoint on that subnet; the proxy also joins a host-scoped shared upstream network. The proxy's DNS and HTTP(S) listeners bind only to its per-user private-network address, so other proxies on the shared upstream cannot invoke its allowlist or attribution identity. Docker private-network membership remains the user-scoped access boundary; the worker receives standard `HTTP_PROXY`/`HTTPS_PROXY` variables but no proxy control plane. The proxy enforces the host-owned hostname allowlist on both CONNECT and TLS SNI, rejects loopback, link-local, metadata, private, and IPv4-mapped IPv6 destinations at connect time, preserves end-to-end TLS, and emits structured request audit records carrying the invocation id without forwarding that correlation header. Opaque TLS tunnels cannot preserve host-mediated HTTP's preflight request-estimate meaning for `NetworkPolicy.max_egress_bytes`; sandbox proxy policy rejects a configured byte ceiling rather than claiming a wire limit it cannot enforce. Local Docker creates one managed proxy and private network per `(tenant, user)` plus one compatible shared upstream network per host. Idle suspension removes the proxy but retains the stopped worker's internal network, preserving its DNS endpoint and container-local state; wake recreates egress on that same private network. Retention removes the stopped worker and its private network. Transport shutdown leaves compatible per-user resources adoptable and never stops another transport's proxy. Railway creates a dedicated two-network proxy shape inside its private outer sandbox for each command; workers never receive unrestricted Railway NAT egress. Ad hoc sandbox transports stay on `docker run --network none` unless composition attaches an explicit broker. A Unix-socket broker exposes `IRONCLAW_REBORN_NETWORK_MODE=brokered`, `IRONCLAW_REBORN_HTTP_BROKER_SOCKET`, and `IRONCLAW_REBORN_HTTP_BROKER_URL` while preserving `--network none`; Unix-socket paths are Unix-host affordances. Secret broker handoff remains metadata-only through `IRONCLAW_REBORN_SECRET_MODE=brokered` plus either `IRONCLAW_REBORN_SECRET_BROKER_SOCKET` or `IRONCLAW_REBORN_SECRET_BROKER_URL`; raw secret material is not injected into command environments. Caller-supplied overrides for reserved broker or proxy environment variables fail closed. Capability-specific host credential injection and response mediation still flow through `ironclaw_sandbox::ScriptRuntimeHttpAdapter<RuntimeHttpEgress>`; the generic shell proxy does not mint secret handles or inject provider credentials.

`IRONCLAW_SANDBOX_MAX_EGRESS_BYTES` is retired for generic shell egress. Setting it fails profile construction because CONNECT tunnels carry opaque TLS bytes and cannot preserve the host-mediated HTTP request-estimate contract.

Local Docker resolves the proxy by immutable image digest and pulls the public image when it is absent. A configured private proxy image must already exist in the daemon because the sandbox runtime does not accept registry credentials.
