...
First-party `builtin.http` keeps inline responses within a model-visible budget through the host-runtime-local tool-call HTTP egress port, while shared `RuntimeHttpEgress` callers keep strict response-limit errors. For successful non-save responses, the transport captures at most 10 MiB of the post-sanitization body. Bodies larger than 16 KiB are retained through the invocation's artifact persistence port and return an 8 KiB-or-smaller preview with `artifact_ref` and `artifact_bytes`; the artifact contains the exact sanitized response bytes, not pre-redaction transport data or the serialized result envelope. Missing artifact persistence fails this large-response path closed. `response_body_limit` remains the bounded model-visible inline budget (48 KiB by default, at most 256 KiB, with smaller positive values honored), while the artifact threshold may select the smaller preview first. Large binary bodies must not be exposed as inline `body_base64`; they return omission metadata while the exact bytes remain in the artifact, and tiny binary bodies may remain base64 for compatibility. HTTP 4xx/5xx bodies are not artifact-retained by this path and continue through the bounded failure-diagnostic contract below. `builtin.http.save` keeps the website-scale 10 MiB default response-body limit for saved bodies, requires `WriteFilesystem`, requires `save_to`, and carries only a host-derived narrowed mount grant for the resolved destination. Network-only `builtin.http` must reject `save_to` before egress. Zero limits/timeouts remain invalid input, and positive timeouts are clamped to the 30s runtime wall-clock ceiling.
