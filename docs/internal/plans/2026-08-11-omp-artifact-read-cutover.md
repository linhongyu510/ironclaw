# Omp Artifact Read Cutover Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove the model-visible `builtin.result_read` tool and recover all truncated capability output through pinned omp `read artifact://<numeric-id>:<selector>`.
...
**Approved design:** `docs/internal/superpowers/specs/2026-08-11-omp-artifact-read-cutover-design.md`
...
|---|---|
| Pinned artifact URI contract | `tests/fixtures/omp_coding_contract/**`, `tests/reborn_omp_coding_contract_snapshot.rs` |
...
| `artifact://` reading | `crates/extensions/ironclaw_extension_support/src/coding/omp/read.rs`, `mod.rs`, engine tests |
...
| Streaming high-output producers | `crates/kernel/ironclaw_host_runtime/src/first_party.rs`, `first_party_tools/shell*.rs`, `first_party_tools/omp.rs` |
...
| Production-shaped coverage | Create `tests/integration/reborn_omp_artifact_read_cutover.rs`; update integration support only where required |
...
**Files:**
- Create: `tests/fixtures/omp_coding_contract/golden/artifact_urls.json`
- Create: `tests/fixtures/omp_coding_contract/output/artifact_protocol.json`
- Modify: `tests/fixtures/omp_coding_contract/manifest.json`
- Modify: `tests/fixtures/omp_coding_contract/provenance.json`
- Modify: `tests/reborn_omp_coding_contract_snapshot.rs`
- Modify: `tests/support/omp_coding_contract/mod.rs`
...
```bash
cargo test --test reborn_omp_coding_contract_snapshot artifact_protocol_cases_are_pinned -- --exact
```
...
The fixture must contain these case IDs and expected strings from omp commit `08819b279cf02ae2545e69dad7111ab48d91d35e`:
...
Copy the two pinned TypeScript sources into `tests/fixtures/omp_coding_contract/sources/` and record their SHA-256 values in provenance. Preserve the upstream MIT license reference already present in the fixture set.
...
```bash
cargo test --test reborn_omp_coding_contract_snapshot
```
...
```bash
git add tests/fixtures/omp_coding_contract tests/reborn_omp_coding_contract_snapshot.rs tests/support/omp_coding_contract/mod.rs
git commit -m "test(coding): pin omp artifact URI contract"
```
...
The omp `read` capability fails closed with an unavailable host error when an `artifact://` input is used without `artifact_reader`; local paths continue to work. Agent-scoped production host construction fails if either unscoped artifact port is absent. An agent-scoped streaming handler fails if `artifact_writer` is absent, and a materialized adapter fails if `accounted_artifact_persister` is absent. Non-loop/admin dispatches may resolve all scoped fields as `None`.
...
### Task 7: Teach omp read to resolve `artifact://`

**Files:**
- Modify: `crates/extensions/ironclaw_extension_support/src/coding/omp/mod.rs`
- Modify: `crates/extensions/ironclaw_extension_support/src/coding/omp/read.rs`
- Modify: `crates/kernel/ironclaw_host_runtime/src/first_party_tools/omp.rs`
- Test: `tests/reborn_omp_coding_engines.rs`
...
async fn read_artifact_selector_matches_pinned_output() {
    let context = omp_context_with_artifact(7, "alpha\nbeta\ngamma\ndelta\n");
    let output = omp::read(&context, json!({"path":"artifact://7:2-3"}))
...
```bash
cargo test --test reborn_omp_coding_engines read_artifact_selector_matches_pinned_output -- --exact
```

Expected: FAIL with path-resolution or not-found output.

- [ ] **Step 3: Add the pre-scoped reader to `OmpEngineContext`**
...
```bash
cargo test --test reborn_omp_coding_engines artifact
cargo test --test reborn_omp_coding_contract_snapshot artifact
cargo test -p ironclaw_host_runtime omp --lib
```
...
```bash
git add crates/extensions/ironclaw_extension_support/src/coding/omp crates/kernel/ironclaw_host_runtime/src/first_party_tools/omp.rs tests/reborn_omp_coding_engines.rs
...
- [ ] **Step 5: Make omp adapters artifact-aware without changing small output**
...
git commit -m "feat(loop): replace result reader with omp artifact reads"
```
...
**Files:**
- Create: `tests/integration/reborn_omp_artifact_read_cutover.rs`
...
```bash
cargo test --test reborn_omp_artifact_read_cutover -- --nocapture
```
...
```bash
cargo test --test reborn_omp_artifact_read_cutover
```
...
```bash
git add tests/integration/reborn_omp_artifact_read_cutover.rs tests/integration/support tests/CLAUDE.md
git commit -m "test(loop): cover omp artifact read cutover"
```
...
cargo test --test reborn_omp_coding_contract_snapshot
cargo test --test reborn_omp_coding_engines
cargo test --test reborn_omp_artifact_read_cutover
cargo test -p ironclaw_architecture_tests
```
...
- Provider tools contain omp `read` and no `result_read` spelling.
...
URLs, SSH, other internal URI schemes, remaining omp tools, or a WebUI artifact
