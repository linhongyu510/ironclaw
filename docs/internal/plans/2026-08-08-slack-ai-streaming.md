...
**Date:** 2026-08-08 (rev. 3 — token-by-token streaming is the deliverable, matching WebUI)
**Status:** Design validated by two independent reviews (architect + critical review, both "proceed with changes"); expanded per implementer decision to full live-text streaming.
...
| Contracts size ceiling | `ironclaw_extension_contracts` at 7,748-line ceiling with ~440 lines headroom (`reborn_dependency_boundaries.rs:610-728`) — fine for this change; re-capture in the same PR if exceeded |
