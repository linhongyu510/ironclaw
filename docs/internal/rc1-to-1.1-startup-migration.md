...
| Extension manifests and activation | rc1 manifests with top-level `[[capabilities]]`; rc1 `installed`/`disabled`/`enabled` state | validated `ironclaw.capability_provider/v1` manifest section and normalized installation activation state | Only during rc1 snapshot import, translate capability declarations and rewrite `product_adapter[.<surface>].host_ingress[*].descriptor.policy.effect_path.type` from `product_workflow` to `product_surface`, covering the bare `product_adapter` section and one immediate surface subsection without recursive descent; ordinary discovery and installation remain fail-closed on retired values. Run the complete 1.1 validator, preserve state, and never widen installed/disabled to enabled | Byte-for-byte retained rc1 snapshot plus canonical compatibility row |
...
| Routines/triggers | backend `trigger_records` repository | same backend repository | No record transform; the artifact canary creates scheduled and paused routines through the rc1 product path and requires exact list/read-back after upgrade, restart, rollback, and re-upgrade | Same IDs, names, schedules, timestamps, and scheduled/paused states |
