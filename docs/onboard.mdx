...
variable. That home is the installation boundary, not a parent for profile-named state:
durable state, host-managed system content, tenant/user workspaces, and runtime
bookkeeping live directly beneath it in `state/`, `system/`, `workspaces/`, and
`runtime/`. Profile changes remain operator-controlled restarts and do not move those
namespaces.
