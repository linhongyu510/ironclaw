...
`[telegram]` section, and no key that enables a channel. See
[Channels](/channels/overview).

`google.*` carries the OAuth client id, redirect URI, and hosted-domain hint. Configure it
from the web interface rather than by hand.
