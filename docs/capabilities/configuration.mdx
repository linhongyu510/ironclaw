...
`IRONCLAW_REBORN_HOME`. This is one installation's storage boundary: a profile
selects runtime policy and process backend, never a profile-named child
directory.
...
The durable namespaces are directly below that home:

```text
<IRONCLAW_REBORN_HOME>/
├── state/       # libSQL state and cached local master key when applicable
├── system/      # host-managed extensions, prompts, and skills
├── workspaces/  # persistent tenant/user workspace leaves
└── runtime/     # provider/runtime bookkeeping and adoption recovery data
```

For a sandboxed request, only
`workspaces/users/<tenant-user-digest>` is mounted as `/workspace`; the Reborn
home, `state/`, `system/`, sibling workspaces, and credentials remain host-side.
...
one user must not run a profile with single-user host semantics. Change it only under
operator control with a full restart: startup compares the requested policy with the
persisted durable-state, tenancy, and workspace-isolation envelope and rejects an unsafe
transition before opening stores or runtimes. Profiles are not freely hot-swappable and
do not select a different storage root. See [Security](/security).
</Warning>
...
Channels are not configured in this file. Slack and Telegram are turned on by installing
their extension in the web interface and completing setup — there is no `[slack]` or
`[telegram]` section, and no key that enables a channel. The retired `[slack]` setup keys
are rejected outright: a config file that still carries them makes `ironclaw serve`
refuse to start, with a migration pointer. See
[Channels](/channels/overview).

`google.*` carries the OAuth client id, redirect URI, and hosted-domain hint. Set it either
from the web interface or with `ironclaw config set google.client_id` /
`google.client_secret` / `google.redirect_uri` — both paths are supported.
...
| Variable | Purpose |
| --- | --- |
| `IRONCLAW_REBORN_HOME` | Installation boundary for configuration and the direct `state/`, `system/`, `workspaces/`, and `runtime/` namespaces |
