...
host egress boundary
  ...
  ├─ reject credential-shaped query params and user:pass@ in the URL
  ├─ leak-scan the URL (raw and percent-decoded), headers, and body
  ├─ inject the staged credential (https only)  ← the ONLY plaintext point
  └─ match scheme + host (+ port) against the tool's declared network
     targets and credential audiences; refuse private-IP ranges
    ↓
external service
    ↓
...
| `[[tools.credentials]]` | The credential to attach for a matching `audience`, and how (`header` with an optional prefix, query parameter, path placeholder, JSON body pointer, HTTP Basic, or VAPID authorization). The tool never receives the value. |
