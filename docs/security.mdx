...
On the way out, the host egress boundary rejects any credential the tool supplied itself (sensitive headers, credential-shaped query parameters, `user:pass@` in the URL), leak-scans the URL, headers, and body, injects the staged credential — the only point where plaintext exists — and matches the destination against the tool's declared network targets and credential audiences, refusing private-IP ranges. On the way back, it strips sensitive response headers, redacts the injected credential's own value, and leak-scans the body before the tool, and then the model, sees anything.
