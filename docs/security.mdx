...
It recognizes 23 secret shapes — provider keys like OpenAI `sk-…` and Anthropic `sk-ant-…`, AWS `AKIA…` and Google `AIza…`, GitHub `ghp_…`, Stripe and Slack tokens, private-key blocks, bearer tokens and JWTs, and IronClaw's own `icsbx_…` sandbox placeholders. Each match triggers one of three responses:
