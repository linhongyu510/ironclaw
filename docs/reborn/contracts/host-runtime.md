...
First-party handlers that need process effects receive execution through the selected `RuntimeProcessPort`. `ProcessBackendKind::UserSandbox` is wired through a typed `RebornRuntimeProcessBinding` that pairs the resolved policy with a `UserSandboxProcessPort`; composition rejects a sandbox profile without this complete binding and rejects the binding on every non-sandbox profile. The sandbox crate derives the persistent workspace/checkpoint identity with `TenantUserWorkspaceKey::from_scope(scope)`: its `digest_segment()` is the single leaf under `IRONCLAW_REBORN_HOME/workspaces/users/<tenant-user-digest>`. Host storage initialization creates the `workspaces/` root; sandbox preparation never creates that configured root through a path lookup. Projects, threads, turns, and transient invocations for one tenant/user share that one leaf, while users in the same tenant and identical user ids in different tenants remain isolated. Container user identity and workspace sharing are explicit sandbox configuration, not composition-layer UID guesses or raw permission-bit assumptions.

Docker user sandbox command mounts are derived from the request `MountView`, never from caller-supplied host paths. The workspace root (`workspaces/`), its `users/` parent, the Reborn home, `state/`, `system/`, `runtime/`, and every sibling tenant/user leaf are host-only directories: none is request-resolvable or a trusted mount-catalog source. The transport admits `/workspace` only for the exact current caller target `/projects/workspace/users/<TenantUserWorkspaceKey::digest_segment()>`, and binds only the matching prepared host leaf. A `/workspace` grant for the root, `users/` parent, a sibling leaf, a descendant/override, or any other host directory fails closed; it may replace the default leaf bind only when it names that exact target. Initial workspace admission opens the `workspace root -> users parent -> exact leaf` chain with no-follow directory handles, verifies containment, and captures that leaf's descriptor identity before bind construction. The bind constructor then re-canonicalizes and verifies the same relationship. Immediately before Docker create, the host re-admits the namespace with no-follow directory handles: the configured workspace root and `users/` parent must be directories owned by the current effective host uid and must not be group- or other-writable, and the selected leaf must be host-owned with the same admitted path and descriptor identity as the bind source. Only that leaf is writable in the sandbox, so sandbox code cannot mutate the parent namespace. Docker receives a path rather than an IronClaw file descriptor; this is therefore an explicit trusted-host namespace boundary, not a claim that the daemon has an fd-pinned mount source. A process able to mutate the effective-uid-owned root or `users/` namespace is trusted host code; untrusted filesystem writers are excluded by the ownership and mode admission and the final recheck. Other mount targets resolve only when trusted composition configured a matching virtual-root-to-host-root mount source. Read-only binds require read + list + execute and no write/delete; writable binds require read + list + execute + write + delete because Docker directory binds cannot enforce delete separately from write. Unsupported permission shapes, unconfigured virtual roots, container system-path aliases, non-directory targets, and source escapes fail closed before container creation.

Railway uses a provider-owned checkpoint keyed by the same typed tenant/user
scope, but checkpoint contents are not portable durable workspace data. Moving
between Railway and Docker, or between Railway environments, requires a
...
The Railway provider canary is deliberately separate and opt-in:
