...
- **Owns:** the extension lifecycle writer and the active-installation snapshot; the loader trio — native, WASM, MCP — and binding checks; activation and removal transactions; the ingress router and the manifest-recipe verifier that mints sealed verified-inbound evidence; egress transports; the generic channel-identity, connection, configuration, and pairing mechanisms every vendor shares. ✎ **Amended 2026-07-31 (#6930), clarified 2026-08-05:** and the **hosted-MCP registration pipeline** — endpoint admission, manifest synthesis for a user-supplied remote server, preparation/discovery, and remote-catalog safety screening. Registration-specific vocabulary and state remain separate from the shared lifecycle: after preparation, the complete package is admitted as a registered package definition with definition-level manager/member visibility, both initialized to the authenticated caller. Registration must not create or mutate an `ExtensionInstallation`, create installation membership, enter credential setup, activate a runtime, or publish capabilities. A later explicit install is the only operation that enters the generic install→setup→activate→remove lifecycle. Future admin management resolves current admin standing at authorization time instead of persisting an admin-id snapshot. Where this pipeline goes when `ironclaw_extension_manager` splits out is an open question — PROPOSAL §6.8.2.
