...
| A Telegram private-chat run starts a native ephemeral draft; an overlapping DM gets an immediate busy notice quoting that DM, and the durable final reply still quotes its own prompt (#6643/#6644) | `extension_delivery.rs::unbound_telegram_actor_pairs_via_web_minted_code_then_turns_attribute_to_the_paired_user` (step 8 + draft and anchored delivery evidence) |
