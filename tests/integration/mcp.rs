...
async fn installed_local_mcp_dispatches_through_production_composition() {
    ...
    h.assert_model_tools_contains("mock-mcp__search")
        .await
        .expect("installed-local MCP capability is disclosed to the model");
    h.assert_tool_invoked("mock-mcp.search")
        .await
        .expect("installed-local MCP capability dispatched");
    h.assert_tool_result_contains("local result")
        .await
        .expect("installed-local MCP result reached the model");
    h.assert_reply_contains("done")
        .await
        .expect("final reply finalized");
