//! Colima-shared tempdir + unique-identity helpers for the sandbox-shell
...
//! `UserSandbox` bind-mount source rooted there becomes a phantom root-owned
...
//! real cause. Rooting explicitly under `$HOME`, or a caller-selected
//! `IRONCLAW_SANDBOX_TEST_TMPDIR`, sidesteps the mount exclusion regardless of
//! whatever `$TMPDIR` the test process happens to inherit.
//!
...
/// A tempdir rooted under `IRONCLAW_SANDBOX_TEST_TMPDIR` when explicitly set,
/// otherwise `$HOME/.ironclaw-test-tmp` (never the process's ambient
/// `$TMPDIR`), for any harness storage root a `UserSandbox` container
/// bind-mounts. The override lets restricted test harnesses choose another
/// Colima-shared writable directory without changing production paths.
pub(crate) fn home_rooted_tempdir(prefix: &str) -> std::io::Result<tempfile::TempDir> {
    let base = match std::env::var_os("IRONCLAW_SANDBOX_TEST_TMPDIR") {
        Some(path) => PathBuf::from(path),
        None => {
            let home = std::env::var_os("HOME").ok_or_else(|| {
                std::io::Error::other("HOME must be set when IRONCLAW_SANDBOX_TEST_TMPDIR is unset")
            })?;
            PathBuf::from(home).join(".ironclaw-test-tmp")
        }
    };
