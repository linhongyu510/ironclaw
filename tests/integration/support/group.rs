...
use super::scripted_provider::{
    ErrLlm, ErrLlmKind, IncompleteAttemptLlm, IncompleteModelAttemptKind,
    IncompleteModelAttemptProbe, ModelProviderCallProbe, ParkingModelGate,
...
/// parked-until-released, bounded recoverable failure, unconditional failure,
/// or incomplete-attempt failure. The enum keeps these modes mutually
/// exclusive by construction, with no priority rule at the dispatch site.
#[derive(Default)]
...
impl<'g> RebornThreadBuilder<'g> {
    ...
    pub async fn build(self) -> HarnessResult<RebornIntegrationHarness> {
        ...
        // `Parked`, `Recoverable`, and `Incomplete` select their corresponding
        // wrappers. `ThreadModelMode` keeps all provider modes mutually
        ...
        let (raw, model_provider_call_probe): (
            ...
            ThreadModelMode::Incomplete(probe) => {
                (Arc::new(IncompleteAttemptLlm::new(probe)), None)
            }
