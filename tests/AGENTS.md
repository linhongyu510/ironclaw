...
| A sandbox-profile canonical loop executes as an unprivileged user in one reusable per-user Docker container; host file tools and sandbox shell share the same scoped workspace in both directions, container-local state persists across calls, threads share the user's container, and a scheduled trigger starts the correct idle user's loop before persisting its reply | `reborn_sandbox_shell_turn.rs` |
