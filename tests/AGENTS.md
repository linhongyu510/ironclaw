...
| A sandbox-profile canonical loop executes as an unprivileged user in one reusable per-user Docker container; host file tools and sandbox shell share the same scoped workspace in both directions while container-local state persists across calls and the container is shared across the user's threads | `reborn_sandbox_shell_turn.rs` |
