...
class NoDuplicateYamlKeysTests(unittest.TestCase):
    ...
    def test_anchored_flow_mapping_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            directory = self._workflows_dir(root)
            (directory / "anchored-flow.yml").write_text(
                "jobs:\n"
                "  build:\n"
                "    with: &upload { if-no-files-found: ignore, "
                "if-no-files-found: error }\n",
                encoding="utf-8",
            )
            errors = validate_no_duplicate_yaml_keys(root)
            self.assertTrue(
                any(
                    "anchored-flow.yml:3" in error and "flow mapping" in error
                    for error in errors
                ),
                errors,
            )
...
class NoEvalInWorkflowRunBlocksTests(unittest.TestCase):
    ...
    def test_eval_after_shell_grouping_tokens_fails(self):
        commands = (
            '( eval "$REPRO" )',
            '{ eval "$REPRO"; }',
            'run() { eval "$REPRO"; }',
            'case "$kind" in test) eval "$REPRO" ;; esac',
        )
