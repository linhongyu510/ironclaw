...
class RebornPrTestPlanTests(unittest.TestCase):
    ...
    def test_sandbox_source_change_selects_docker_lane(self) -> None:
        repository = metadata()
        repository["workspace_members"].append("host-runtime")
        repository["packages"].append(
            {
                "id": "host-runtime",
                "name": "ironclaw_host_runtime",
                "manifest_path": str(
                    ROOT / "crates/ironclaw_host_runtime/Cargo.toml"
                ),
            }
        )
        repository["resolve"]["nodes"].append(
            {"id": "host-runtime", "deps": []}
        )
        plan = planner.build_plan(
            event="pull_request",
            changed_paths=[
            ...
            metadata=repository,
            canonical_packages=["ironclaw_host_runtime"],
        )
        self.assertEqual(plan["mode"], "selected")
        self.assertTrue(plan["run_sandbox_docker"])
    ...
    def test_unrelated_package_does_not_select_sandbox_docker_lane(self) -> None:
        plan = self.plan("pull_request", ["crates/alpha/src/lib.rs"])
        self.assertFalse(plan["run_sandbox_docker"])
