...
IGNORED_PREFIXES = (
    ...
    # .github/workflows/openwiki-update.yml, never hand-edited). Prose,
    # like `docs/` above: it selects no Rust lane.
    "openwiki/",
    # `test-tools/` is standalone tooling documentation and helper scripts,
    # outside the cargo workspace; no Reborn test lane builds it.
    "test-tools/",
)
...
QA_HARNESS_PREFIXES = (
    ...
    # directories that differ only by hyphen-vs-underscore. Only the hyphen
    # form was listed, so a PR touching the underscore one fell through to the
    # fail-closed arm. Keep both; do not "fix the typo" by deleting either.
    "scripts/live-canary/",
    "scripts/live_canary/",
    "scripts/reborn_qa_matrix/",
    "scripts/reborn_webui_v2_live_qa/",
)
# `tests/e2e/reborn_*` harnesses fail closed by design (they are shared
# fixtures, not one scenario), which forces a per-file decision rather than a
# blanket prefix. These are the files whose decision has been made: the
# browser harness is executed only by `reborn-e2e.yml`, which has its own
# `paths: tests/e2e/**` trigger and scope detection. No Reborn Rust lane
# imports or runs it, so it selects nothing here.
DECIDED_E2E_HARNESS_PATHS = {
...
PR_STATIC_CONTROL_PATHS = {
    ...
    #   * the workspace lint/format/dependency/release policy files are owned
    #     by the lanes that read them — `clippy.toml` by Code Style, `deny.toml`
    #     by cargo-deny, `release-plz.toml` by the release workflow. None is
    #     read by a Reborn test lane. They sit beside `Cargo.toml` above, which
    #     was already classified for exactly this reason. All three exist on
    #     this tree; this set is fail-closed, so only real files are listed.
    "clippy.toml",
    "deny.toml",
    "release-plz.toml",
    ...
    *,
    ...
    for path in sorted(paths):
        ...
        if path.startswith("tests/fixtures/llm_traces/reborn_qa/") or path in {
            ...
            continue
        if path in DECIDED_E2E_HARNESS_PATHS:
        ...
        if path.startswith("tests/e2e/"):
            ...
            continue
        if path.startswith("crates/") and "/" not in path[len("crates/") :]:
            # A file sitting directly in `crates/` belongs to no package —
            # `crates/AGENTS.md` and `crates/Architecture.md` are the tree-wide
            # prose map. Prose selects no Rust lane (same reasoning as `docs/`
            # in IGNORED_PREFIXES), and the crate arm below would fail closed
            # on it because no package directory can contain it. Family
            # directories introduced by the WS6/WS7 moves (`crates/app/`,
            # `crates/extensions/`) are unaffected: their contents have a
            # further `/` and still resolve through `package_directories`.
            reasons.append(f"tree-wide crates/ prose owns: {path}")
            continue
        if path.startswith("crates/"):
            ...
            if package is None:
                if not (ROOT / path).exists():
                    # The path was *removed* by this diff — the old side of a
                    # crate rename or a family-directory move. It maps to no
                    # package because the package no longer lives there, and a
                    # file that no longer exists can select no lane; the new
                    # side of the same rename is in this diff too and does the
                    # selecting. Without this arm every rename PR fails closed
                    # here, which is the whole of WS6 and all of WS7.
                    reasons.append(f"removed by this diff: {path}")
                    continue
